...
const ReservasScreen = ({ navigation }: Props) => {
  ...
  const renderItem = ({ item }: { item: Reserva }) => (
    <View style={styles.card}>
      ...
      <Text style={styles.description}>Check-in: {item.data_checkin}</Text>
      <Text style={styles.description}>Check-out: {item.data_checkout}</Text>
      <Text style={styles.description}>
        Hóspedes: {item.quantidade_hospedes}
      </Text>
      <Text style={styles.description}>
        Valor total: R$ {item.valor_total}
      </Text>
      <Text style={styles.description}>Cliente ID: {item.cliente}</Text>
      <Text style={styles.description}>Hospedagem ID: {item.hospedagem}</Text>
      <Text style={styles.description}>
        Pagamentos selecionados: {item.pagamento?.length ?? 0}
      </Text>
      <TouchableOpacity
